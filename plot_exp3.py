...
# read analysis (x_a)
x_a = np.zeros((2,401,40))
mn  = 'im' # Choose 'pm' for perfect model, 'im' fpr imperfect model.
x_a[0,:,:] += np.genfromtxt('txt_file_new/exp3_1_x_a_'+mn+'.txt')
x_a[1,:,:] += np.genfromtxt('txt_file_new/exp3_2_x_a_'+mn+'.txt')
...
plt.title('RMSE & Bias of Experiment 3',fontsize=17,fontweight='bold',loc='left')
#plt.title('${Perfect\ Model}$',fontsize=16,loc='right')
plt.title('${Imperfect\ Model}$',fontsize=16,loc='right')
plt.savefig('plot_new/verify_exp3_'+mn+'.png',dpi=300)
plt.tight_layout()
plt.show()
